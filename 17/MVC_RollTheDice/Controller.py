...
class Controller:
    def __init__(self, window):
        ...
        self.oTitleDisplay = pygwidgets.DisplayText(
            window, (330, 30), "Roll The Dice!", fontName="monospaces", fontSize=34
        )
        self.oQuitButton = pygwidgets.TextButton(
            window, (20, 595), "Quit", width=100, height=35
        )

        self.oRoundsDisplay = pygwidgets.DisplayText(
            window,
            (260, 600),
            "Number of rolls:",
            fontName="monospaces",
        ...
        self.oRoundsInput = InputNumber(
            window,
            (430, 600),
            value=str(N_ROUNDS_AT_START),
            fontName="monospaces",
            fontSize=28,
            width=100,
            initialFocus=True,
            keepFocusOnSubmit=True,
            allowFloatingNumber=False,
            allowNegativeNumber=False,
        )

        self.oRollDiceButton = pygwidgets.TextButton(
            window, (690, 595), "Roll Dice", width=100, height=35
        )

        self.oDiceImage = pygwidgets.Image(window, (650, 15), IMAGE_PATH + "twoDice.png")
        # This area reserved for the different views
        self.viewArea = pygame.Rect(45, 70, WINDOW_WIDTH - 90, WINDOW_HEIGHT - 200)

        self.oBarButton = pygwidgets.TextRadioButton(
            window, (80, 540), "View", "Bar Chart", value=True, fontSize=36
        )
        self.oPieButton = pygwidgets.TextRadioButton(
            window, (350, 540), "View", "Pie Chart", fontSize=36
        )
        self.oTextButton = pygwidgets.TextRadioButton(
            window, (620, 540), "View", "Text", fontSize=36
        )
    ...
    def handleEvent(self, event):
        ...
        if self.oRollDiceButton.handleEvent(event) or self.oRoundsInput.handleEvent(
            event
        ):
            self.generateNewData()
            self.oView.update()
    ...
    def generateNewData(self):
        ...
        try:
            ...
            pyghelpers.textYesNoDialog(
                self.window,
                pygame.Rect(170, 180, 430, 170),
                msg,
                "OK",
                None,
