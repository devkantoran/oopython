...
def showCustomAlertDialog(theWindow, theText):
    oDialogBackground = pygwidgets.Image(theWindow, (60, 80), IMAGE_PATH + "dialog.png")
    ...
    oOKButton = pygwidgets.CustomButton(
        theWindow,
        (355, 235),
        IMAGE_PATH + "okNormal.png",
        over=IMAGE_PATH + "okOver.png",
        down=IMAGE_PATH + "okDown.png",
        disabled=IMAGE_PATH + "okDisabled.png",
    )
    response = pyghelpers.customYesNoDialog(
        theWindow, oDialogBackground, oPromptDisplayText, oOKButton, None
    )
    return response
...
def showCustomYesNoDialog(theWindow, theText):
    ...
    oNoButton = pygwidgets.CustomButton(
        theWindow,
        (95, 235),
        IMAGE_PATH + "noNormal.png",
        over=IMAGE_PATH + "noOver.png",
        down=IMAGE_PATH + "noDown.png",
        disabled=IMAGE_PATH + "noDisabled.png",
    )
    oYesButton = pygwidgets.CustomButton(
        theWindow,
        (355, 235),
        IMAGE_PATH + "yesNormal.png",
        over=IMAGE_PATH + "yesOver.png",
        down=IMAGE_PATH + "yesDown.png",
        disabled=IMAGE_PATH + "yesDisabled.png",
    )
    response = pyghelpers.customYesNoDialog(
...
def showCustomAnswerDialog(theWindow, theText):
    ...
    oUserInputText = pygwidgets.InputText(
        theWindow, (225, 165), "", fontSize=36, initialFocus=True
    )
    oNoButton = pygwidgets.CustomButton(
        theWindow,
        (105, 235),
        IMAGE_PATH + "cancelNormal.png",
        over=IMAGE_PATH + "cancelOver.png",
        down=IMAGE_PATH + "cancelDown.png",
        disabled=IMAGE_PATH + "cancelDisabled.png",
    )
    oYesButton = pygwidgets.CustomButton(
        theWindow,
        (375, 235),
    ...
    response = pyghelpers.customAnswerDialog(
        theWindow,
        oDialogBackground,
        oPromptDisplayText,
        oUserInputText,
...
oTitle = pygwidgets.DisplayText(
    window, (150, 25), "Click all buttons to test dialogs", fontSize=36, textColor=WHITE
)
oResults = pygwidgets.DisplayText(
    window, (20, 450), "", fontSize=36, textColor=WHITE, width=600
)
...
while True:
    ...
    for event in pygame.event.get():
        ...
        if oTextAlertButton.handleEvent(event):
            ignore = pyghelpers.textYesNoDialog(
                window, (75, 80, 500, 150), "This is an alert!", "OK", None
            )
        ...
        if oTextYesNoButton.handleEvent(event):
            returnedValue = pyghelpers.textYesNoDialog(
                window, (75, 80, 500, 150), "Do you want fries with that?"
            )
            if returnedValue:
        ...
        if oCustomYesNoButton.handleEvent(event):
            returnedValue = showCustomYesNoDialog(
                window, "Do you want fries with that?"
            )
            if returnedValue:
                oResults.setValue("User clicked the Yes button")
            else:
        ...
        if oTextAnswerButton.handleEvent(event):
            userAnswer = pyghelpers.textAnswerDialog(
                window,
                (75, 80, 500, 200),
        ...
        if oCustomAnswerButton.handleEvent(event):
            userAnswer = showCustomAnswerDialog(
                window, "What is your favorite flavor of ice cream?"
            )
            if userAnswer is not None:
                oResults.setValue("User clicked OK, text was: " + userAnswer)
            else:
